...
```
Sprint:         S0 · Foundation closeout
Brief:          plan/sprints/S0-foundation.md
Sub-task:       13 of 13 COMPLETE — formal exit pending three billing-tag activations and completion of the screenshot evidence pack
Pause rating:   🟡 AMBER
Live billable:  none
...
Next action:    Activate `Sprint`, `Ephemeral`, and `TeardownBy` when Billing discovers them; add the missing S0 screenshots, then start S1
Blocked on:     Non-blocking: Billing must discover `Sprint`, `Ephemeral`, and `TeardownBy`; activate when visible
```
...
|---|---|---|---|---|---|
| ◐ | S0 Foundation | — | $1 | implementation complete; closeout pending | pending posting |
...
### S0 — implementation complete · formal closeout pending
...
- **Open exit items:** Billing has not yet exposed `Sprint`, `Ephemeral`, and `TeardownBy` for activation. The required budget, model-access, and paired Streamlit/CloudWatch screenshots are not yet stored in the S0 evidence directory; PNG files remain local-only under the sprint brief's `.gitignore` rule.
